...
- **Project-specific skills:** If the target repo has its own frontend task skill (e.g., `.ai/frontend-task/`), check for it first. That skill may provide component inventory, styling tokens, conventions, and workflow guides. Defer to it for implementation details (imports, token usage, test patterns) — this skill handles the Figma→code translation, the project-specific skill handles the coding standards.
