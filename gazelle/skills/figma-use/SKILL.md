...
**Use ONLY the official Figma MCP** (`mcp__figma__*` or `mcp__claude_ai_Figma__*`). Do NOT use TalkToFigma (`mcp__TalkToFigma__*`) — it is deprecated for this workflow. If the cloud Figma MCP (`claude_ai_Figma`) disconnects mid-session, fall back to the local official plugin (`mcp__figma__`), not TalkToFigma.

## 12. Visual Taste Guide

Before generating UI, check if your project has a style guide at `context-training/style.md` (or similar). A good style guide codifies:

- **Hierarchy rules** — squint test, visual weight system
- **Color usage** — when to use brand vs accent vs neutral colors
- **Spacing rhythm** — context-dependent density
- **AI treatment** — distinct but harmonious, trust mechanisms
- **Anti-patterns** — what your product should NEVER look like
- **Component composition** — the standard page skeleton

This is NOT optional. Designs that skip the taste guide will produce generic output.

## 13. Project Design System Reference (Appendix)

> If your project has a known Figma file with cached component keys, add them here as a project-specific appendix.
> Use cached keys instead of calling `search_design_system` for known components.
> Only call `search_design_system` for components NOT listed in your project's reference.
>
> To populate this section for your project, inspect your Figma file's design system libraries, component keys, color tokens, typography, and spacing scale. Document them here so agents can import components by key without repeated `search_design_system` calls.

### DS Component Import Pattern

Standard pattern for importing and using DS components:

```js
// 1. Import component set (for components with variants)
const buttonSet = await figma.importComponentSetByKeyAsync(
  "YOUR_COMPONENT_SET_KEY",
);

// 2. Find the specific variant
const filledAccent = buttonSet.children.find(
  (v) => v.name.includes("Variant=Filled") && v.name.includes("Color=Accent"),
);

// 3. Create instance
const btn = filledAccent.createInstance();

// For single components (icons):
const iconComponent =
  await figma.importComponentByKeyAsync("YOUR_COMPONENT_KEY");
