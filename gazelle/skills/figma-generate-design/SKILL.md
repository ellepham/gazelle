...
> **Before calling `search_design_system`**, check the DS reference appendix in the `figma-use` skill (Section 12) for cached component keys. Only call `search_design_system` for components NOT listed in the reference. This reduces API calls significantly.
